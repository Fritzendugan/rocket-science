...
public class WayPoint extends BaseObject implements ContactListener
{
	...
	public WayPoint(final Body b, final Sprite s, final short mk, final short rk) 
	{
		super(b,s, ObjectKeys.WAY_POINT, mk, rk);
	...
	public static WayPoint Load(final DataInputStream inp, final Section section, final PhysicsWorld world, final short mk, final short rk) throws IOException
	{
		final float x, y;
		...
		return Create(x, y, angle, angularVelocity, world, section, mk, rk);
	}
	
	public static WayPoint Create(final float x, final float y, final float ang, final float angVel,
									 final PhysicsWorld pw, final Section section, final short myKey,
									 final short relKey)
	{
		...
		return new WayPoint(body, sprite, myKey, relKey);
	}
	...
	public static ObjectLoadingAdapter getLoadingAdapter()
	{
		...
		{
			@Override
			public BaseObject load(DataInputStream inp, final short mk, final short rk) throws IOException 
			{
				return WayPoint.Load(inp, ObjectLoader.currentSection, ObjectLoader.physicsWorld, mk, rk);
			}	
		};
	}
